...
#extract the first 8 digits and return as yyyy-mm-dd
sub fixdate
{
	my $d = shift;
	return undef unless(defined $d);
	$d =~ s/[^0-9]//g;
	$d = sprintf "%4d-%02d-%02d",substr($d,0,4),substr($d,4,2),substr($d,6,2);
	return $d;
}
...
sub new
{
	...
	my %cusips = $g->get_cusips;
	$self->{cusip2ticker} = \%cusips;
	...
	$self->{dtstart} = undef;			# the starting date for this file
	$self->{dtend} = undef;				# the ending date for this file
	$self->{dtbefore} = undef;			# the day before dtstart
	...
	$self->{asof} = undef;				# the "as of" date for this file
...
{
	my $self = shift;
	return fixdate($self->{asof});
}

# return a day before dtstart
sub dtbefore
{
	my $self = shift;
	return fixdate($self->{dtbefore});
}

sub dtstart
{
	my $self = shift;
	return fixdate($self->{dtstart});
}

sub dtend
{
	my $self = shift;
	return fixdate($self->{dtend});
}
...
{
	my $self = shift;
	$self->die("Bad signon status: " . $self->{val}) unless ($self->{val} eq "SUCCESS");
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_STATUS_MESSAGE
{
	my $self = shift;
	$self->die("Bad statement status: " . $self->{val}) unless ($self->{val} eq "SUCCESS");
}
...
{
	my $self = shift;
	$self->die("Not from SCHWAB.COM " . $self->{val}) unless ($self->{val} eq "SCHWAB.COM");
}
...
sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_DTSTART
{
	my $self = shift;
	$self->{val} =~ s/[^0-9]//g;
	$self->{dtstart} = substr($self->{val},0,8);
	my $t = Time::Piece->strptime($self->{dtstart}, "%Y%m%d")-86400;
	$self->{dtbefore} = $t->strftime("%Y%m%d");
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_DTEND
{
	my $self = shift;
	$self->{val} =~ s/[^0-9]//g;
	$self->{dtend} = substr($self->{val},0,8);
}
...
sub OPEN
{
	...
	if ($self->{pos} =~ /INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_$self->{tag}/ &&
		$self->{tag} !~ /INCOME|BUYOTHER|TRANSFER|BUYDEBT|BUYMF|BUYSTOCK|SELLSTOCK|SELLDEBT|SELLMF|SELLOPT|BUYOPT|INVBANKTRAN|MARGININTEREST|SELLOTHER/)
	{
...
sub OPEN_INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN
{
	my $self = shift;
	$self->{event} = {
...
sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN_STMTTRN_FITID
{
	my $self = shift;
	$self->{event}->{fitid} = $self->{val};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_BUYMF_INVBUY_SUBACCTFUND
{
	my $self = shift;
	die "unexpected subaccount {$self->{val}}" unless ($self->{val} eq "CASH");
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN_STMTTRN_DTPOSTED
{
	my $self = shift;
	my ($y,$m,$d) = $self->{val} =~ /(\d{4})(\d{2})(\d{2})/;
...
sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN_STMTTRN_TRNTYPE
{
	my $self = shift;
	$self->{event}->{action} = "RECVINT" if ($self->{val} eq 'INT');
	$self->{event}->{action} = "RECVDIV" if ($self->{val} eq 'DIV');
	$self->{event}->{action} = "CASHOUT" if ($self->{val} eq 'CASH');
	$self->{event}->{action} = "CASHOUT" if ($self->{val} eq 'FEE');
	$self->{event}->{action} = "CASHOUT" if ($self->{val} eq 'SRVCHG');
	$self->{event}->{action} = "CASHIN"  if ($self->{val} eq 'DEP');
	$self->{event}->{action} = "CASHIN"  if ($self->{val} eq 'CREDIT');
	$self->{event}->{action} = "CASHOUT" if ($self->{val} eq 'DEBIT');
	$self->die("unknown income type") unless (defined $self->{event}->{action});
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN_STMTTRN_TRNAMT
{
	my $self = shift;
...
sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN_STMTTRN_MEMO
{
	my $self = shift;
	$self->{event}->{memo} = $self->{val};
}

sub CLOSE_INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_INVBANKTRAN
{
	my $self = shift;
	if ($self->{event}->{action} eq 'CASHIN' ||
		$self->{event}->{action} eq 'CASHOUT')
	{
		$self->{event}->{shares} = $self->{event}->{money};
	}
	#NOTE: resist the temptation to "correct" CASHIN to RECVINT based on
	#Bank Interest in memo because it screws up the sharebalance in 'cash'
...
{
	my $self = shift;
	$self->{event} = {acctkey => $self->{acctkey}};
}
...
# selldebt transactions

sub OPEN_INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT
{
	my $self = shift;
	$self->{event} = {acctkey => $self->{acctkey}};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_INVTRAN_FITID
{
	my $self = shift;
	$self->{event}->{fitid} = $self->{val};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_INVTRAN_DTTRADE
{
	my $self = shift;
	my ($y,$m,$d) = $self->{val} =~ /(\d{4})(\d{2})(\d{2})/;
	$self->{event}->{juncture} = "$y-$m-$d";
...
sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_INVTRAN_MEMO
{
	my $self = shift;
	$self->{event}->{memo} = $self->{val};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_SECID_UNIQUEID
{
	my $self = shift;
	$self->{event}->{uniqueid} = $self->{val};
	$self->{event}->{cusip} = $self->{val};
...
sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_UNITS
{
	my $self = shift;
	$self->{event}->{shares} = $self->{val};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_UNITPRICE
{
	my $self = shift;
	$self->{event}->{quote} = $self->{val};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_INVSELL_TOTAL
{
	my $self = shift;
	$self->{event}->{money} = $self->{val};
}

sub INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT_SELLREASON
{
	my $self = shift;
	$self->{event}->{reason} = $self->{val};
}

sub CLOSE_INVSTMTMSGSRSV1_INVSTMTTRNRS_INVSTMTRS_INVTRANLIST_SELLDEBT
{
	my $self = shift;
	if ($self->{event}->{reason} eq 'MATURITY')
	{
		$self->{event}->{action} = 'MATURED';
	}
	elsif ($self->{event}->{reason} eq 'CALL')
	{
		$self->{event}->{action} = 'CALLED';
	}
	else
	{
		$self->{event}->{action} = 'SELL';
	}
	$self->{event}->{quote} = 100.0 if ($self->{event}->{quote} == 0.0);
	$self->{event}->{shares} = -$self->{event}->{money} if ($self->{event}->{shares} == 0.0);
	push @{$self->{events}},$self->{event};
}
...
sub CLOSE_OFX
{
	...
	foreach my $s ($self->securities)
	{
		...
		if (!defined $s->{ticker})
		{
			$s->{ticker} = $self->{cusip2ticker}->{$s->{cusip}}
				if (defined $s->{cusip});
	...
	{
		$self->die("Event is missing uniqueid!") if (!defined $e->{uniqueid});
		$e->{ticker} = $self->{cusip2ticker}->{$e->{cusip}}
			if (!defined $e->{ticker});
		$self->die("Event is missing ticker!") if (!defined $e->{ticker});
		$self->die("Event is missing juncture!") if (!defined $e->{juncture});
		$self->die("Event is missing acctkey!") if (!defined $e->{acctkey});
		$self->die("Event is missing action!") if (!defined $e->{action});
		...
		if (!defined $e->{quote})
		{
			...
			if (!defined $e->{quote})
			{
				...
				{
					die ("Event is missing quote");
				}
			}
		}
		$self->die("Event is missing money!") if (!defined $e->{money});
		$self->die("Event is missing shares!") if (!defined $e->{shares});
		$self->die("Event is missing quote!") if (!defined $e->{quote});

		if (defined($e->{fitid}))
		{
			$e->{evthash} = $g->get_evthash($e->{fitid});
		}
		else
		{
	...
	{
		$self->die("Position is missing uniqueid!")
			if (!defined $p->{uniqueid});
		$p->{ticker} = $self->{cusip2ticker}->{$p->{cusip}}
			if (!defined $p->{ticker});
		$self->die("Position is missing ticker!") if (!defined $p->{ticker});
	}
}
